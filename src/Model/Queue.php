...
public function __construct(  $maxSize=1000  ){$this->maxSize=$maxSize;}
...
usort($this->items,function($a,$b){return $b['priority']-$a['priority'];});
}
...
{
$this->items=array_values(array_filter($this->items,function($entry)use($callback){return call_user_func($callback,$entry['data']);}));
}
}
