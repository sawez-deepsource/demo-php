...
foreach($data as $row){$output.='<item>';
    foreach($row as $key=>$value){$output.='<'.$key.'>'.$value.'</'.$key.'>';}
$output.='</item>';}
$output.='</root>';
}
    return $output;
...
$cleaned=[];
        ...
        {
if(is_string($value)){$cleaned[$key]=trim(strip_tags($value));}
elseif(is_array($value)){$cleaned[$key]=$this->validateAndClean($value);}
            elseif(is_numeric($value)){$cleaned[$key]=(float)$value;}
